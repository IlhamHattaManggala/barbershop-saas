                            ...
                            <div class="p-6 sm:p-8 bg-zinc-900/90 border border-amber-500/30 space-y-6 shadow-xl font-sans">
                                ...
                                <form wire:submit.prevent="createBooking" class="space-y-4 text-xs">
                                    ...
                                    <div>
                                        ...
                                        <input type="date" wire:model.live="reservation_date" required class="w-full bg-zinc-950 border border-amber-500/30 px-4 py-3 text-xs text-amber-100 focus:border-amber-400 focus:outline-none transition" />
                                    </div>

                                    <div class="space-y-1.5 font-mono">
                                        <label class="block text-[11px] text-amber-300 uppercase tracking-wider">Slot Jam Cukur (Live Available Slots)</label>
                                        <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 max-h-48 overflow-y-auto p-1 border border-amber-500/30 bg-zinc-950">
                                            @forelse($available_slots as $slot)
                                                <button 
                                                    type="button" 
                                                    wire:click="selectSlot('{{ $slot['time'] }}')" 
                                                    disabled="{{ !$slot['available'] }}"
                                                    class="py-2.5 px-2 text-xs font-bold transition-all border flex flex-col items-center justify-center cursor-pointer disabled:cursor-not-allowed {{ $start_time === $slot['time'] ? 'bg-amber-400 text-zinc-950 border-amber-400 shadow-[0_0_12px_rgba(251,191,36,0.4)]' : ($slot['available'] ? 'bg-zinc-900 border-amber-500/30 text-amber-100 hover:border-amber-400' : 'bg-zinc-950 border-zinc-900 text-zinc-700 line-through opacity-50') }}"
                                                    title="{{ $slot['reason'] }}"
                                                >
                                                    <span>{{ $slot['time'] }}</span>
                                                    <span class="text-[9px] font-normal {{ $start_time === $slot['time'] ? 'text-zinc-900' : ($slot['available'] ? 'text-amber-400' : 'text-zinc-700') }}">{{ $slot['available'] ? 'Tersedia' : $slot['reason'] }}</span>
                                                </button>
                                            @empty
                                        ...
                                        @error('start_time')
                                            <span class="text-[11px] font-semibold text-amber-500 mt-1 block">{{ $message }}</span>
                                        @enderror
                                    </div>
